...
# Create a line plot of Phoenix and LA weekly NO2 levels
ggplot(weekly_la_phx, aes(x = week, y = mean_no2, color = city)) + geom_line(size = 1.2) + 
  geom_point(size = 2.5) + 
  
  # Add California lock down start date line 
  geom_vline(xintercept = as.Date("2020-03-19"),
             linetype = "dashed",
             color = "gray",
             size = 1
             ) + 
  # Add California start date label 
...
**Parallel trends assumption has been violated. Even before the lock down there were extreme trend differences. We cannot use difference in difference to compare them.**
...
**It seems parallel trend assumptions are not violated with this example. New Mexico and Texas follow a similar trend before the lock down so we can use them to analyze the impact of the lackdown order**
...
did_data_nm_tx <- did_data_nm_tx %>% 
  mutate(
    date_obj = mdy(date),
    # Treat = New Mexico, Control = Texas
...
```{r}
trt_effect <- (y_trt_post - y_trt_pre) - (y_ctrl_post - y_ctrl_pre)

print(paste("The treatment effect is", round(trt_effect, 4)))
```

This New Mexico population received -3.51 less NO2 concentration parts per billion per month relative to the Texas population. 
...
summary(tx_nm_did)
```
Usually for difference in difference you only have to interpret the interaction term
...
**I think Texas makes a good counter factual to the neighboring city in New Mexico since they are neighboring cities with similar weather conditions with different lock down policies. However, since we are only dealing with 3 months of data these NO2 estimates are variable. Our model would be better at picking up on these trends if we had more data for a longer amount of time.**
...
**The interaction term treatment:is_post represents the treatment effect. It is not statistically significant at 0.05. We fail to reject the null hypothesis that the lock down has no effect on the concentration of NO2 parts per billion for the months of February, March, and April.**
...
**A neighboring city in Texas serves as a counterfactual. If the effect of the duststorm is the same for both cities, the DiD estimate is not ruins since both the control and treatment experience the same conditions.**
...
**The DiD estimate would get smaller because both cities experience a reduction is NO2 because they both start staying home around the same time regardless of the reason for it.**