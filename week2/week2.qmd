...
```
Intercept: Without any treatment, we expect the number of treated piles to be ~78 on average. 

Treatment: The treatment reduced the average number of trash piles by approximately 25 piles. 
...
```{r}
check_model(m1_ols, check = "qq") # This output tells us we should move to poisson since dots dont fall along the line 
```
...
Flatter density curve so it is over dispersed
...
Intercept: On average, the model estimates our control group to have 78 waste piles. 

Coefficient: The model estimates that the treatment reduced waste burning by 32% in treated neighborhoods. 

Calculation: ((e^-0.38) - 1) * 100  = 32 % 
...
```
Intercept: On average our model estimates the control piles to have 74 waste piles. 

Coefficient: On average, the model estimates that the treatment reduced waste burning by 34% in treated neighborhoods. 
...
m2_change = (exp(-0.38) - 1) * 100 # Change in POTS # -31.6
...
There is potentially already selection bias with this experiment. The pre_count_avg shows that before the treatment group already had a lower waste pile average before the experiment began. So maybe the different is due to this.  
...
export_summs(m3_log, m4_control, 
             model.names = c("Log OLS Regression", "Log OLS Regression with a control variable"), statistics = "none")
```

Intercept: With a pre-treatment average of zero the waste piles will be about 1.59 on average 

Treatment: When accounting for the pre-treat average, the treatment group has 24% less waste piles on average 
