...
```{r}
tbl_2way <- lobsters %>% 
  # Calculate total cpue for each year and region
...
```{r}
tbl_3way <- lobsters %>% 
  # Calculate total cpue for each year/region/trt group
...
fancy_table <- tbl_3way %>% 
  gt(rowname_col = "year") %>% 
  tab_header(
    title = "European Lobster Catch by Region and Year and Treatment",
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "cow"); beep(13)  
