...
data_binned <- sim_data %>% 
  mutate(distance_bin = cut(distance, breaks = seq(-20, 20, by = 1), inlcude.lowest = TRUE)) %>% 
...
The two lines represent the correlation between distance from a protected area and deforestation rate. Negative values are outside the protected area and positive values are inside. 
...
If there was no discontinuity it would mean protected areas are not really affecting deforestation rate. 
...
**RESPONSE**:
Protected area reduce deforestation on average by 10% 

Interaction coefficient: The slope in protected areas would be 2% higher than the slope in unprotected area.
...
**RESPONSE**: 
This local polynomial is optimizing a smaller band width around the cut off. We therefore have less data for this estimation. This is only data from -2 to 2. This number was found by an optimization algorithm. 

The advantages associated with this local polynomial regression are reduced bias, it doesnt have to be linear. 
...
**RESPONSE**:
The binned linear plot shows a more severe drop  at the cut off line while the local polynomial plot has a smaller drop. The local polynomial plot follows the data more closely. 
...
final_plot <- wrap_plots(plot_list) + 
  plot_layout(ncol = 2) + 
  plot_annotation(title = "Deforestation Rate",
